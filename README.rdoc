...
Well, it's Rails-based local map application with <s>blackj</s>search and POI categories using OpenStreetMap data.
...
You may tune bundled styles, which are generated using excellent {TileMill}[http://mapbox.com/tilemill/] application from {here}[https://github.com/alno/mapstyle].
...
Why bundling them? There are two different reasons:
