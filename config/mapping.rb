...
polygons :objects do
  ...
  after_import do
    conn.exec "INSERT INTO #{name}(id,type,type_array,name,center,tags) SELECT osm_id, #{type_mapping}, #{type_array_mapping}, #{name_mapping}, way, tags FROM raw_osm_point src WHERE #{conditions}"
