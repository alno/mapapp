polygons :places do
  ...
  after_import do
    conn.exec "INSERT INTO #{name}(id,type,name,center) SELECT osm_id, #{type_mapping} AS type, #{name_mapping} AS name, way FROM raw_osm_point src WHERE #{conditions} AND (SELECT COUNT(1) FROM #{name} ep WHERE ep.geometry && src.way AND ST_Contains(Geometry(ep.geometry), src.way)) = 0"
    conn.exec "UPDATE #{name} SET tags = #{name}.tags || src.tags, center = src.way FROM raw_osm_point src WHERE #{conditions} AND #{name_mapping} = #{name}.name AND geometry && way && ST_Contains(Geometry(geometry), way)"
  end
end
